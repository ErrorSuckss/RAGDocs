...
const handleSubmit = async() => {
    ...
    try {
        ...
        if(response.status === 201){
            ...
            objFiles.value.meta.total += 1
            console.log('added total: ' + objFiles.value.meta.total)
        }
    } catch (error) {
        console.log(error);
        ...
        uploadForm.value.errors = error.response?.data?.errors;
            ...
            <div class="mx-auto max-w-6xl space-y-8 px-6 py-8">
                ...
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="mt-1 text-sm text-slate-500">Upload and manage your documents.</p>
                    </div>
                    <label for="file"
                        class="inline-flex cursor-pointer items-center gap-2 rounded-md bg-indigo-600 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-indigo-500">
                ...
                <div class="rounded-xl border border-slate-200 bg-white shadow-sm">
                    <div class="border-b border-slate-100 px-6 py-4">
                        <h2 class="text-sm font-semibold text-slate-900">Upload a file</h2>
                    </div>
                    ...
                    <form class="space-y-6 px-6 py-6" @submit.prevent="handleSubmit">
                        <div>
                            ...
                            <input ref="fileInput" type="file" name="file" id="file" class="sr-only" @change="onFileChange" />
                            <label for="file"
                                class="group flex min-h-[168px] cursor-pointer flex-col items-center justify-center rounded-lg border-2 border-dashed border-slate-200 bg-slate-50/50 px-6 py-10 text-center transition hover:border-indigo-300 hover:bg-indigo-50/40">
                                <div class="mb-3 flex h-12 w-12 items-center justify-center rounded-full bg-white text-slate-400 shadow-sm ring-1 ring-slate-200 transition group-hover:text-indigo-500 group-hover:ring-indigo-200">
                                    <svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M7 16a4 4 0 0 1-.88-7.9A5.5 5.5 0 0 1 17.5 8h.5a4 4 0 0 1 0 8" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" />
                                        <path d="M12 12v7m0-7 3 3m-3-3-3 3" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" />
                                    </svg>
                                </div>
                                <span class="text-sm font-medium text-slate-700">
                                    Drag &amp; Drop to upload or <span class="text-indigo-600">Browse Files</span>
                                </span>
                            ...
                            <p v-if="uploadForm.errors?.file" class="mt-1.5 text-sm text-red-600">
                                {{ uploadForm.errors.file[0] }}
                            </p>
                        </div>

                        <div v-if="uploadForm.file" class="flex items-center justify-between rounded-md border border-slate-200 bg-slate-50 px-4 py-3">
                            <div class="flex min-w-0 items-center gap-2.5">
                                <svg class="h-4 w-4 shrink-0 text-slate-400" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <button @click.prevent="uploadForm.file = null; $refs.fileInput.value = null" :disabled="disabled"
                                class="shrink-0 rounded p-1 text-slate-400 transition hover:bg-slate-200 hover:text-slate-600 disabled:opacity-50">
                        ...
                        <button type="submit" :disabled="disabled"
                            class="inline-flex w-full items-center justify-center gap-2 rounded-md bg-indigo-600 py-2.5 px-6 text-sm font-semibold text-white transition hover:bg-indigo-500 disabled:cursor-not-allowed disabled:bg-slate-300 sm:w-auto">
                            <svg v-if="disabled" class="h-4 w-4 animate-spin" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
